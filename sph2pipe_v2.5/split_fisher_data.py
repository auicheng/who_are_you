...
def process_file(wav_file_dir, transcript_dir_prefix, output_dir):
    for filename in os.listdir(wav_file_dir):
        ...
        for i in range(len(transcript_data)):
            ...
            start_time_millisecs = int(float(transcript_data_line[0]) * 100000)       #unit is in microseconds
            end_time_millisecs = int(float(transcript_data_line[1]) * 100000)         #unit is in microseconds
            ...
            chunk_data.export(output_dir+"/"+chunk_name, format="wav")       #saves each split file to output directory here
