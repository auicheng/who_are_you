...
class abi_data(Dataset):


    def __init__(self, dataset, label, padlen=7251, featlen=40, encoder=None):
        self.pad_len = padlen
    ...
    def __get_abi_material__(self):
        ...
        for line in all_files:
            if 'ABI' in line[0] and line[1].strip() == self.dataset:
    ...
    def __load_features_and_labels__(self, label_column):
        ...
        self.file_descriptions = self.__get_abi_material__()
        file_label = []
...
def main_biRNN():
    ...
    # Data description
    input_size = 7251  # 2939, 20 # TODO: Determine this from the data
    ...
    train = abi_data('train', label, padlen=input_size, featlen=sequence_length)
    print('ABI train data items: {}'.format(train.__len__()))

    dev = abi_data('dev', label, encoder=train.get_encoder())
    print('ABI dev data items: {}'.format(dev.__len__()))

    test = abi_data('test', label, encoder=train.get_encoder())
    print('ABI test data items: {}'.format(test.__len__()))
    ...
    # Save the Model
    # torch.save(rnn.state_dict(), 'abi.pkl')
